...
def assign_families(spe_name, TE_lib, blast_80_80, outfile):
    ...
    SeqIO.write(final_tes, outfile + "_808080_recovered.fa", "fasta")#added by atsanchez for TEammo
...
def rename_duplicates(input_fasta, output_fasta):
    seen_ids = {}
    duplicates = {}

    # Read the input FASTA file and track duplicate IDs
    with open(input_fasta, "r") as input_handle:
        records = list(SeqIO.parse(input_handle, "fasta"))
        for record in records:
            if record.id in seen_ids:
                if record.id not in duplicates:
                    duplicates[record.id] = 1
                else:
                    duplicates[record.id] += 1
                name = record.id.split("#")[0]
                classification = record.id.split("#")[1]
                new_id = f"{name}_duplicate{duplicates[record.id]}#{classification}"
                record.id = new_id
                record.description = ''
...
if __name__ == '__main__':
    ...
    elif option == "rename_duplicates":
        if len(sys.argv) == 4:
            input_fasta_path = sys.argv[2]
            output_fasta_path = sys.argv[3]

            rename_duplicates(input_fasta_path, output_fasta_path)
        else:
            print("ERROR in parameters, usage: python3 " + sys.argv[
                0] + " rename_duplicates TE_library.fasta TE_library_without_duplicates.fasta")
            sys.exit(0)
    else:
